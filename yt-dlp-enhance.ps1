...
function Save-Settings {
    ...
    try {
        ...
        Write-Host "   💾 Settings saved for next time!" -ForegroundColor Green
    }
    catch {
        Write-Host "   ⚠️  Could not save settings" -ForegroundColor Yellow
    }
}
...
function yt-dlp {
    ...
    if ($savedSettings -and -not $useDefaults) {
        ...
        elseif ($settingChoice -ne "2") {
            Write-Host "   ✓ Using saved settings`n" -ForegroundColor Green
        }
        else {
            $savedSettings = $null
        }
    ...
    if ($savedSettings -and $savedSettings.ytDlpPath -and (Test-Path $savedSettings.ytDlpPath)) {
        ...
        if (-not $ytDlpPath) {
            Write-Host "⚠️  yt-dlp.exe not found in common locations" -ForegroundColor Yellow
            ...
            switch ($ytChoice) {
                "1" {
                    ...
                    if ($customPath -and (Test-Path $customPath)) {
                        ...
                        Write-Host "   ✓ Found: $ytDlpPath" -ForegroundColor Green
                    }
                    else {
    ...
    elseif ($savedSettings -and $settingChoice -eq "1") {
        ...
        switch ($cookieChoice) {
            "2" { 
                ...
                Write-Host "   ✓ Using Chrome cookies" -ForegroundColor Green
            }
            "3" { 
                $cookieArgs = "--cookies-from-browser firefox"
                Write-Host "   ✓ Using Firefox cookies" -ForegroundColor Green
            }
            "4" { 
                $cookieArgs = "--cookies-from-browser edge"
                Write-Host "   ✓ Using Edge cookies" -ForegroundColor Green
            }
            "5" {
                $cookieFile = Read-Host "   Enter path to cookie file"
                if ($cookieFile -and (Test-Path $cookieFile)) {
                    $cookieArgs = "--cookies `"$cookieFile`""
                    Write-Host "   ✓ Using cookie file" -ForegroundColor Green
                } else {
                    Write-Host "   ⚠️  Cookie file not found, continuing without cookies" -ForegroundColor Yellow
                }
            }
            "6" {
                ...
                try {
                    ...
                    Write-Host "   ❌ Cookie export failed: $_" -ForegroundColor Red
                }
            }
        }
        Write-Host ""
    ...
    elseif ($savedSettings -and $settingChoice -eq "1") {
        ...
        switch ($uaChoice) {
            "2" {
                $userAgentArgs = '--user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"'
                Write-Host "   ✓ Using Chrome User-Agent" -ForegroundColor Green
            }
            "3" {
                $userAgentArgs = '--user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:121.0) Gecko/20100101 Firefox/121.0"'
                Write-Host "   ✓ Using Firefox User-Agent" -ForegroundColor Green
            }
            "4" {
                $customUA = Read-Host "   Enter custom User-Agent"
                if ($customUA) {
                    $userAgentArgs = "--user-agent `"$customUA`""
                    Write-Host "   ✓ Using custom User-Agent" -ForegroundColor Green
                }
            }
        }
        Write-Host ""
    ...
    elseif ($savedSettings -and $settingChoice -eq "1") {
        ...
        if ($archiveChoice -eq "2") {
            $archiveFile = "$downloadPath\download-archive.txt"
            $archiveArgs = "--download-archive `"$archiveFile`""
            Write-Host "   ✓ Using archive: $archiveFile" -ForegroundColor Green
            Write-Host "   💡 Videos in this file will be skipped" -ForegroundColor Gray
        }
        Write-Host ""
    }
    ...
    switch ($choice) {
        ...
        "10" {
            Write-Host "`n🎮 Stream to VLC Player..." -ForegroundColor Yellow
            ...
            if ($vlcPath) {
                Write-Host "   ✓ Found VLC: $vlcPath" -ForegroundColor Green
                Write-Host "   🎬 Starting stream..." -ForegroundColor Cyan
                $streamCommand = "$ytDlpPath -o - --downloader ffmpeg -f 'bv*+ba/b' $cookieArgs $userAgentArgs $url"
                Write-Host "   Executing: $streamCommand | vlc -" -ForegroundColor Gray
                Invoke-Expression "$streamCommand | & `"$vlcPath`" -"
                return
            } else {
                Write-Host "   ❌ VLC not found! Please install VLC Media Player." -ForegroundColor Red
        ...
        "12" {
            ...
            $finalCommand = "$ytDlpPath -o ""$downloadPath\$outputTemplate"" $cookieArgs $userAgentArgs $archiveArgs $customCommand $url"
        }
        default {
    ...
    try {
        ...
        if ($errorMsg -match "403|Forbidden") {
            Write-Host "`n💡 TIP: Got 403 error? Try using browser cookies!" -ForegroundColor Cyan
        ...
        elseif ($errorMsg -match "429|Too Many Requests") {
            Write-Host "`n💡 TIP: Rate limited! Try:" -ForegroundColor Cyan
        ...
        elseif ($errorMsg -match "codec|ffmpeg") {
            Write-Host "`n💡 TIP: FFmpeg issue detected!" -ForegroundColor Cyan
...
function yt-reset-settings {
    ...
    if (Test-Path $settingsFile) {
        ...
        Write-Host "✓ Settings reset! Next run will prompt for new settings." -ForegroundColor Green
    }
    else {
        Write-Host "⚠️  No saved settings found." -ForegroundColor Yellow
    }
}

# Help function
function yt-help {
    Write-Host "`n🎬 YT-DLP QUICK REFERENCE" -ForegroundColor Magenta
    Write-Host "═════════════════════════════════════════════════" -ForegroundColor DarkGray
    ...
    Write-Host "═════════════════════════════════════════════════" -ForegroundColor DarkGray
    Write-Host "`n💡 Settings are saved automatically!" -ForegroundColor Cyan
    Write-Host "   Next time: Choose 'Use saved settings' to skip prompts`n" -ForegroundColor Gray
}

Write-Host "✓ yt-dlp-enhance loaded! " -ForegroundColor Green -NoNewline
